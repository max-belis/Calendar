...
def get_duration(year_value, month_index):
    if month_index in [3, 5, 8, 10]:  # Апрель, Июнь, Сентябрь, Ноябрь
        duration = 30
    elif month_index == 1:  # Февраль
...
def print_days(days_in_month, start_day):
    print('   ' * start_day, end='')  # Отступ перед первым числом
    for day in range(1, days_in_month + 1):
        if day < 10:
            print(day, end='  ')
        else:
            print(day, end=' ')
        if (day + start_day) % 7 == 0:  # Конец недели
            print()
    # Если месяц не закончился воскресеньем, добавляем перенос строки
...
# Главная функция: печать календаря на весь год.
def print_calendar(year):
    ...
    for month_number in range(12):
        ...
        # Готовим start_day для следующего месяца
